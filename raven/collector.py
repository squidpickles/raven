...
class SerialClient(asyncore.dispatcher):
    def __init__(self, device="/dev/ttyUSB0", speed=115200):
        ...
        self.looper = threading.Thread(
            target=asyncore.loop, name="Asyncore loop", kwargs={"timeout": kLoopTimeout}
        )
    ...
    def create_socket(self, device, speed):
        ser = serial.Serial(
            device, speed, serial.EIGHTBITS, serial.PARITY_NONE, timeout=0.5
        )
        ser.nonblocking()
        ser.flushInput()
        ser.flushOutput()
...
class Collector(SerialClient):
    ...
    def handle_read(self):
        ...
        try:
            self.read_callback(
                raven.parser.RavenMessageParser.parse(self.recv_buffer.strip(b"\x00"))
            )
        except Exception:
            pass
        else:
